...
public class ConnectionData
{
    ...
    private ImmutableMap<Identifier, String> channels;

    /* package private */ ConnectionData(Map<String, Pair<String, String>> modData, Map<Identifier, String> channels)
    {
     ...
     */
    public ImmutableMap<Identifier, String> getChannels()
    {
        return channels;
    }
     ...
     */
    public record ModMismatchData(Map<Identifier, String> mismatchedModData, Map<Identifier, Pair<String, String>> presentModData, boolean mismatchedDataFromServer)
    {
        /**
         ...
         */
        /*public static ModMismatchData channel(Map<Identifier, String> mismatchedChannels, ConnectionData connectionData, boolean mismatchedDataFromServer)
        {
            Map<Identifier, String> mismatchedChannelData = enhanceWithModVersion(mismatchedChannels, connectionData, mismatchedDataFromServer);
            Map<Identifier, Pair<String, String>> presentChannelData = getPresentChannelData(mismatchedChannels.keySet(), connectionData, mismatchedDataFromServer);
         ...
         * @param connectionData The connection data instance responsible for collecting the server mod data.
         */
        /*public static ModMismatchData registry(Multimap<Identifier, Identifier> mismatchedRegistryData, ConnectionData connectionData)
        {
            List<Identifier> mismatchedRegistryMods = mismatchedRegistryData.values().stream().map(Identifier::getNamespace).distinct().map(id -> new Identifier(id, "")).toList();
            Map<Identifier, String> mismatchedRegistryModData = mismatchedRegistryMods.stream().map(id -> ModList.get().getModContainerById(id.getNamespace()).map(modContainer -> Pair.of(id, modContainer.getModInfo().getVersion().toString())).orElse(Pair.of(id, NetworkRegistry.ABSENT))).collect(Collectors.toMap(Pair::getLeft, Pair::getRight));
            Map<Identifier, Pair<String, String>> presentModData = getServerSidePresentModData(mismatchedRegistryModData.keySet(), connectionData);
         ...
         */
        /*private static Map<Identifier, String> enhanceWithModVersion(Map<Identifier, String> mismatchedChannels, ConnectionData connectionData, boolean mismatchedDataFromServer)
        {
         ...
         */
        /*private static Map<Identifier, Pair<String, String>> getPresentChannelData(Set<Identifier> mismatchedChannelsFilter, ConnectionData connectionData, boolean mismatchedDataFromServer)
        {
            Map<Identifier, String> channelData;
         ...
         */
        /*private static Pair<Identifier, Pair<String, String>> getPresentModDataFromChannel(Identifier channel, ConnectionData connectionData, boolean mismatchedDataFromServer)
        {
         ...
         */
        private static Map<Identifier, Pair<String, String>> getServerSidePresentModData(Set<Identifier> mismatchedModsFilter, ConnectionData connectionData)
        {
            Map<String, Pair<String, String>> serverModData = connectionData != null ? connectionData.getModData() : Map.of();
            Set<String> modIdFilter = mismatchedModsFilter.stream().map(Identifier::getNamespace).collect(Collectors.toSet());
            return serverModData.entrySet().stream().filter(e -> modIdFilter.contains(e.getKey())).collect(Collectors.toMap(e -> new Identifier(e.getKey(), ""), Entry::getValue));
        }
    }
}
